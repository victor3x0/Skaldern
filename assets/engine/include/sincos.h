#ifndef __SINCOS_H_INCLUDE__
#define __SINCOS_H_INCLUDE__
...
inline int8_t COS(uint8_t A) {
    return sine_wave[(uint8_t)(A + 64u)];
}

inline void upoint_translate_angle(upoint16_t *point, uint8_t angle, uint8_t speed) {
    point->x += ((int16_t)(SIN(angle) * (speed)) >> 7);
    point->y -= ((int16_t)(COS(angle) * (speed)) >> 7);
}
